...
Motion::Motion(Preferences *p, Scanfile *s)
{
	...
	click = NULL;
	for (i=0; i<3; i++)
		RefVec[i].setValue(0.0, 0.0, 0.0);
	mTomoToStack.makeIdentity();
	...
	for (i=0; i<10; i++)
	{
	    animation_trafo[i].makeIdentity();
	...
	for (i=0; i<10; i++)
	{
		for (j=0; j<18; j++)
		{
			LEDofObject[i][j] = pref->LEDofObjectP[j];
			oldLEDofObject[i][j] = pref->LEDofObjectP[j];
		}
		for (j=0; j<6; j++)
		{
			ObjectsAndReferences[i][j] = pref->ObjectsAndReferencesP[j];
			oldObjectsAndReferences[i][j] = pref->ObjectsAndReferencesP[j];
		}
/*		for (j=0; j<18; j++)
		{
			LEDofObject[i][j] = j/3;
...
char Motion::read_j3dfile(char *filename)//jaws3D file(means .mvm file?)
{
    unsigned char  *buf;
    int            fileSize;
    FILE           *readfile;
    ...
    // read header
    // -----------
    fread(raw_header, 198, 1, readfile);
    ...
    for (pos=0; pos<num_steps; pos++)
    {
        ...
        for (j=0; j<6; j++)
		{
	        ...
	        for (i=0; i<3; i++)
	        {
	            if (word[i] != 0x8000)                 // falls Wert gueltig
				{				                       // falls Bit[14] = 1
		            if ((j==0)&&(i==0)&&((word[i]&0x4000)!=0))
       			       click[0].set1Value(pos, 1);	   // d.h. Knacken
                    word[i] = word[i] & 0x3FFF;        // Bit[14] = Bit[15] = 0 
       		        if ((word[i] & 0x2000) != 0)       // falls Bit[13] = 1 
  		               word[i] = word[i] | 0xC000;     // Bit[14] = Bit[15] = 1 
                    val = word[i];
					vd = led[j][pos];
                    vd[i] = 13.0*double(val)/1000.0; // CCD-units -> mm
	...
	if (err1 != NULL)
	{
		while (err1 != NULL)
		{
			...
			while ((pos < num_steps) &&
				...
				pos++;
			if (pos >= num_steps) // Letzter Schritt unbrauchbar -> kuerzen
			{
	            for (i=0; i<6; i++)
			...
			else
			{
				...
				while ((pos >= 0) &&
					   ((led[err2->led][pos][0] == 10000.0) ||
					    ...
					    (led[err2->led][pos][2] == 10000.0)))
					pos--;
				if (pos < 0)  // Alle Schritte am Anfang unbrauchbar -> kuerzen
				{
	                for (i=0; i<6; i++)
						led[i].deleteValues(0, err2->step+1);
						oldled[i].deleteValues(0, err2->step+1);
					num_steps -= (err2->step+1);
					while (err2 != NULL) // Restliche Fehlermeldungen loeschen
					{
						delete err2;
						err2 = err1;
						if (err1 != NULL)
...
void Motion::ComputeMaxMean()
{
    ...
    for (i=0; i<6; i++)
    {
       ...
       max_leds[i] /= double(num_steps);//num_steps = number of samples
	}
}
...
void Motion::ComputeRefMean()
{
    ...
    for (i=0; i<6; i++)
    {
       ref_leds[i].setValue(0.0, 0.0, 0.0);//set ref_leds to (0,0,0)
       for (pos=0; pos<num_steps; pos++)//sums up led[i][pos]
          ref_leds[i] += led[i][pos];//for loop ends


       ref_leds[i] /= double(num_steps);// look for the average [6*(i), each has 3 values
	}
}


void Motion::FixedXYZ(SdMatrix &rot, double a, double b, double c) //Transformation matrix
{
	//from the old co-system to new co-system
...
void Motion::Vec3ToMatrix(SbVec3d &origo, SbVec3d &px, SbVec3d &pxy, SdMatrix &m)
{
   SbVec3d base[3]; //base is 3*3 matrix
   base[0] = px - origo;
   base[1] = pxy - origo;
   base[2] = base[0].cross(base[1]);
   base[1] = base[2].cross(base[0]);
   base[0].normalize();
   base[1].normalize();
   base[2].normalize();
...
void Motion::TomoToSpheres()
{
    ...
    int            i;
    int            stack;
    SbVec3d        ResVec[3]; //ResVec is 3*3 matrix
...
void Motion::Vec2BaseTrans(SbVec3d base_from[3], SbVec3d base_to[3],SbVec3d from_origin, SbVec3d to_origin, SdMatrix &m)
{
   SbVec3d v1;
...
void Motion::oneStepBackward()
{
	...
	if ((animation_step - floor(animation_step)) > 0.9) // runden
...
char Motion::getClick(char up)
{
    ...
    while (i != animation_step)
	{
	   ...
	   {
		   c = 1;
		   i = animation_step; // ein Knacken genuegt
	   }
	}
	return c;
}
...
void Motion::goToStep(double step)
{
    ...
    if (animation_step < 0)
		animation_step = 0;
    if (animation_step >= num_steps)
		animation_step = num_steps-0.11; // etwas weniger als num_steps
}


void Motion::CalcUVWtoXYZ(SbVec3d &uvw, SbVec3d &xyz)
{
   SdMatrix mUVWToTomo;

   mUVWToTomo = mTomoToUVW.inverse();
   mUVWToTomo.multMatrixVec(uvw, xyz);
}


void Motion::CalcXYZtoUVW(SbVec3d &xyz, SbVec3d &uvw)
{
   mTomoToUVW.multMatrixVec(xyz, uvw);
}
...
char Motion::read_mvmfile(char * filename)
{
	...
	sprintf(pref->loadfilename, filename);	
	//060815 save load filename
...
void Motion::load_mvmfile(const char *filename)
{
   ...
   {
	   // alles zurücksetzen zuruecksetzen?
	   return;
   }
...
void Motion::calc_animationTrafo4(double dStep, int objnr, int refnr) //Transformation? (Matrix?)
{
   SdMatrix m1, m2;
   SbVec3d tmpled[6];
   int camsysnr, i, j;
